    ...
    return {
      onOpen(_, ws) {
        try {
          ...
          const game =
            InProgressGames.find((game) => game.users.has(userID)) ??
          ...
          ws.send(
            JSON.stringify({ error: "Failed to initialize game connection" }),
          );
        }
      },
      ...
      onClose(_, ws) {
        const socket = ws.raw as ServerWebSocket<WebSocketData>;
        console.log(
          `User ${socket.data?.userID} disconnected from game ${socket.data?.gameID}`,
        );
      },
    };
  }),
